    ...
    private static class SVGHandler extends DefaultHandler {
        ...
        public void startElement(String namespaceURI, String localName, String qName, Attributes atts) throws SAXException {
            ...
            if (localName.equals("svg")) {
                ...
                popTransform();
            } else if (!hidden && localName.equals("text-path")) {
            	String points = getStringAttr("d", atts);
            	int[] startPoint = getFirstPoint(points);
            	int[] lastPoint = getLastPoint(points);
            	
            	int y = (int)((startPoint[1]+lastPoint[1])/1.5);
            	int x = (int)((startPoint[0]+lastPoint[0])/1.5);
            	
            	if( lastPoint[1] > startPoint[1] )            	
            		points = reversePath(points);
            	else if( lastPoint[0] < startPoint[0] )            	
            		points = reversePath(points);
            		
            	Path p = doPath(points);
            	String text = getStringAttr("text", atts);
            	pushTransform(atts);
            	Properties props = new Properties(atts);
            	
            	int strokeWidth = Integer.parseInt(getStringAttr("stroke-width", atts));
            	int fontSize = Integer.parseInt(getStringAttr("font-size", atts));
            	...
            	if (doFill(props, gradientMap)) {
            		...
            		canvas.drawTextOnPath(text, p, strokeWidth, strokeWidth, paint);
            	}
            	if (doStroke(props)) {
            		paint.setColor(Color.RED);
            		paint.setTextSize(fontSize);
        ...
        private String reversePath(String p) {
        	...
        	boolean pointDone = false;
         	for( int i = 0 ; i < p.length(); i++ ) {
         		...
         		points.add(new String[] {x,y});
        	}
         	String str = "";
         	for( int i = points.size()-1 ; i > -1 ; i-- ) {
         		String[] tmp = points.get(i);
         		if( i == points.size()-1 ) {
         			tmp[0] = "M" + tmp[0].substring(1,tmp[0].length());
         		}
         		else if( i == 0 ) {
         			tmp[0] = "L" + tmp[0].substring(1,tmp[0].length());
         		}
         		str+= tmp[0] + " " + tmp[1] + " ";
         	}
         	return str;
        ...
        private int[] getFirstPoint(String str){
        	int i = 0;
        	...
        	int[] point = {Integer.parseInt(x),Integer.parseInt(y)};
        	
        	return point;
        }
        
        private int[] getLastPoint(String str){
        	int i = str.length()-1;
        	...
        	String reverseX = new StringBuffer(x).reverse().toString();
        	String reverseY = new StringBuffer(y).reverse().toString();
        	
        	int[] point = {Integer.parseInt(reverseX),Integer.parseInt(reverseY)};
